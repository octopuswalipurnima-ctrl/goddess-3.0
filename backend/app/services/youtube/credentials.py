...
Manages up to 4 rotated YouTube API credentials, tracks per-key failure states,
handles quota-aware cooldowns, prevents infinite retry loops, and publishes
credential recovery and failure events to the Event Bus with zero secret leakage.
"""
    ...
    def is_usable(self, now: float) -> bool:
        ...
        if self.state == CredentialState.COOLDOWN or self.state == CredentialState.FAILED:
            if self.cooldown_until_timestamp and now >= self.cooldown_until_timestamp:
                ...
                logger.info(f"YouTube credential '{self.key_id}' cooldown expired. State restored to AVAILABLE.")
                return True
            return False
        return True
...
class YouTubeCredentialManager:
    ...
    def get_credential(self) -> Tuple[str, str]:
        ...
        raise CredentialUnavailableError(
            "No active YouTube API credentials available (all unconfigured or in cooldown)."
        )
    ...
    async def mark_failed(
        ...
        if key_id in self._slots:
            ...
            # Quota errors trigger longer base cooldowns (min 300s) with exponential backoff
            ...
            logger.warning(
                f"YouTube credential '{key_id}' failed ({slot.last_error_type}): {sanitized_msg}. "
