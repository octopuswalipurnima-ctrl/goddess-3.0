"""
Centralized Credential & Quota Manager for YouTube Data API v3.

Manages up to 4 rotated YouTube API credentials, tracks failure states, handles cooldowns,
and publishes credential recovery and failure events to the Event Bus.
"""
...
from app.services.youtube.models import CredentialHealth, CredentialState

logger = get_logger("youtube.credentials")


class CredentialSlot:
    """Internal representation of a YouTube API credential slot."""
    ...
    def is_usable(self, now: float) -> bool:
        ...
        if self.state == CredentialState.COOLDOWN or self.state == CredentialState.FAILED:
            if self.cooldown_until_timestamp and now >= self.cooldown_until_timestamp:
                ...
                logger.info(f"Credential '{self.key_id}' cooldown expired. State restored to AVAILABLE.")
                return True
            return False
        return True
...
class YouTubeCredentialManager:
    """Centralized manager for rotated YouTube API credentials."""

    def __init__(self, keys: Optional[List[str]] = None):
        self._slots: Dict[str, CredentialSlot] = {}
    ...
    def reload_credentials(self, keys: Optional[List[str]] = None) -> None:
        ...
        raw_keys = keys if keys is not None else [
            settings.youtube_api_key_1,
            settings.youtube_api_key_2,
            settings.youtube_api_key_3,
            settings.youtube_api_key_4,
        ]

        self._slots.clear()
        for idx in range(4):
            key_id = f"youtube-key-{idx + 1}"
            raw_key = raw_keys[idx] if idx < len(raw_keys) and raw_keys[idx] else ""
            self._slots[key_id] = CredentialSlot(key_id, raw_key.strip() if raw_key else "")

        active_count = sum(1 for s in self._slots.values() if s.state == CredentialState.AVAILABLE)
        logger.info(f"Loaded {active_count}/4 active YouTube credentials.")
    ...
    def get_credential(self) -> Tuple[str, str]:
        ...
        raise CredentialUnavailableError("No active YouTube API credentials available (all unconfigured or in cooldown).")
    ...
    async def mark_failed(
        ...
        Publishes YOUTUBE_CREDENTIAL_FAILED event.
        """
        if key_id in self._slots:
            slot = self._slots[key_id]
            ...
            # Quota errors trigger longer cooldowns (e.g. 5 minutes or custom duration)
            duration = cooldown_seconds if not is_quota else max(cooldown_seconds, 300)
            slot.cooldown_until_timestamp = now + duration
            slot.state = CredentialState.COOLDOWN

            logger.warning(
                f"Credential '{key_id}' failed: {error}. Placed in COOLDOWN for {duration}s."
            )

            await event_bus.publish(
                "YOUTUBE_CREDENTIAL_FAILED",
                {
                    "key_id": key_id,
                    "error": error,
                    "is_quota": is_quota,
...
# Global singleton instance of YouTubeCredentialManager
youtube_credentials = YouTubeCredentialManager()
